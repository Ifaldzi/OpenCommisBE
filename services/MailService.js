...
class MailService {
  ...
  async sendNotification(to, subject, body, attachments = null) {
    const mail = await this.transporter.sendMail({
      ...
      html:
        body ||
        "Ada pesanan masuk, segera lakukan konfirmasi sebelum 3 hari kedepan",
      attachments,
    });
  ...
  async sendOrderSubmission(orderData, { filePath, link }, description) {
    ...
    await this.sendNotification(
      ...
      `<h3>Pesananan mu dengan order-id: ${orderData.id} telah dikirimkan oleh illustrator</h3><p>Catatan dari illustrator: ${description}</p><p>${submisionNote}</p><br><p>*Bila terdapat permasalahan terkait hak cipta dikemudian hari dapat diselesaikan langsung dengan ilustrator yang bersangkutan, OpenCommiss tidak memiliki hak maupun kewajiban apapun terkait permasalahan tersebut</p>`,
      attachments
    );
  }
  ...
  async sendAccountVerificationApproval(to, accepted) {
    const body = accepted
      ? "<h3>Pengajuan verifikasi akun anda telah disetujui, anda akan melihat tanda centang biru setelah nama anda</h3>"
      : "<h3>Mohon maaf pengajuan verifikasi akun anda tidak disetujui, anda dapat kembali mengirimkan permohonan verifikasi akun di lain waktu</h3>";
